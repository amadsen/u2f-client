...
// devicePathU2fSupportMap caches information about whether a device with
// a given path supports U2F.
var devicePathU2fSupportMap = {};

function devicesFromCache(){
  return Object.keys(devicePathU2fSupportMap)
    .filter(function (devicePath) {
      return !!devicePathU2fSupportMap[devicePath].isCompatible;
    })
    .map(function (devicePath) {
      return devicePathU2fSupportMap[devicePath].deviceInfo;
    });
}
...
U2FHIDDevice.enumerate = function enumerate(acceptFn) {
    var allDevices = hid.devices(),
        matchedDevicePaths = {},
        newDevices = allDevices.filter(function(deviceInfo){
          var found = !!(devicePathU2fSupportMap[deviceInfo.path]);
          if(found){
            matchedDevicePaths[deviceInfo.path] = true;
          }
          return !found;
        });

    // remove device paths from our devicePathU2fSupportMap if they are not
    // currently found in the system.
    Object.keys(devicePathU2fSupportMap).forEach(function(cachedDevicePath){
      if(!matchedDevicePaths[cachedDevicePath]) {
        delete devicePathU2fSupportMap[cachedDevicePath];
    ...
    if(newDevices.length > 0){
      ...
      newDevices.filter(function(deviceInfo) {
          ...
          devicePathU2fSupportMap[deviceInfo.path] = {
