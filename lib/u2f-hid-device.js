...
var U2FHIDDevice = module.exports = function U2FHIDDevice(rawDevice, initCb) {
    ...
    self._reportSize = 64;
    self._packetBuf = new Buffer(this._reportSize);

    self._queue = [];
    self._curTransaction = undefined;

    self._onError = this._onError.bind(self);
    self._onData  = this._onData.bind(self)
    self.device.on('error', this._onError);  // 'error' should be first because polling starts when 'data' is bound
    self.device.on('data', this._onData);
