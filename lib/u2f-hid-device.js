...
var U2fDevices = require('bindings')('U2fDevices.node'),
...
//     release: 819,
//     interface: -1,
//     usagePage: 61904, // <-- current versions of node-hid may not report usagePage and usage!!!
//     usage: 1
// }
U2FHIDDevice.enumerate = function enumerate(acceptFn) {
    //   v-- use U2fDevices.supportsU2f(vendorId, productId) to check whether a
    //       given device supports the U2F usagePage & usage.
    //   v-- use U2fDevices.devices() to get a pre-filtered
    //       list of devices which support the U2F usagePage & usage.
    var allU2fDevices = U2fDevices.devices(),
        compatibleDevices = [];

    if(allU2fDevices.length > 0){
      compatibleDevices = allU2fDevices.reduce( function getPaths(devices, deviceInfo){
        return devices.concat( hid.devices(deviceInfo.vendorId, deviceInfo.productId) );
      }, []).reduce( function(uniqueDevices, deviceInfo){
        var add = true;
        for (var i = 0, l = uniqueDevices.length; i<l; i++) {
          if(deviceInfo.path === uniqueDevices[i].path){
            add = false;
            break;
          }
        }
        ...
        var isCompatible = (
          (deviceInfo.usagePage === U2FHIDDevice.FIDO_USAGE_PAGE && deviceInfo.usage == U2FHIDDevice.FIDO_USAGE_U2FHID) ||
          U2fDevices.supportsU2f(deviceInfo.vendorId, deviceInfo.productId)
        );
        ...
        // Add unique identifier to deviceInfo to help distinguishing them.
        deviceInfo.id = deviceInfo.path;
        // Make sure the usagePage and usage are set
        deviceInfo.usagePage = U2FHIDDevice.FIDO_USAGE_PAGE;
...
U2FHIDDevice.open = function open(deviceInfo, cb) {
    ...
    try {
      ...
      console.error('Error attempting to open', deviceInfo.path);
