...
// deviceU2fSupportMap caches information about whether a device with
// a given path supports U2F.
var deviceU2fSupportMap = {};

function devicesFromCache(){
  return Object.keys(deviceU2fSupportMap)
    .filter(function (deviceKey) {
      return !!deviceU2fSupportMap[deviceKey].isCompatible;
    })
    .map(function (deviceKey) {
      return deviceU2fSupportMap[deviceKey].deviceInfo;
    });
}

function getDeviceKey (deviceInfo) {
    return deviceInfo.vendorId + ':' + deviceInfo.productId;
}
...
U2FHIDDevice.enumerate = function enumerate(acceptFn) {
    var allDevices = hid.devices(),
        matchedDeviceKeys = {},
        newDeviceSet = allDevices.reduce(function(newDeviceSet, deviceInfo){
          var deviceKey = getDeviceKey(deviceInfo),
              found = !!(deviceU2fSupportMap[deviceKey]);
          if(found){
            matchedDeviceKeys[deviceKey] = true;
          } else if(!newDeviceSet[deviceKey]){
            newDeviceSet[deviceKey] = deviceInfo;
          }
          return newDeviceSet;
        },{}),
        newDevices = Object.keys(newDeviceSet).map(function(deviceKey){
          return newDeviceSet[deviceKey];
        });

    // remove device paths from our deviceU2fSupportMap if they are not
    // currently found in the system.
    Object.keys(deviceU2fSupportMap).forEach(function(cachedDeviceKey){
      if(!matchedDeviceKeys[cachedDeviceKey]) {
        delete deviceU2fSupportMap[cachedDeviceKey];
        // TODO: consider triggering a remove event if we are polling
      }
    });

    if(newDevices.length > 0){
      console.log('Checking new devices for u2f devices:\n', newDevices);
      newDevices.filter(function(deviceInfo) {
          var rawDevice;

          if(!(deviceInfo.hasOwnProperty('usagePage') && deviceInfo.hasOwnProperty('usage'))) {
            console.log('\n', deviceInfo.product);
            try {
                // Trying to get new device's HID Report Descriptor so we can get
                // the Usage Page and Usage.

                // open device
                rawDevice = new hid.HID(deviceInfo.vendorId, deviceInfo.productId);

                // try reading from it - what do we get?
                // also try getting a feature report for report id 0 - what do we get?
                var report = new Buffer(rawDevice.getFeatureReport(0, 64));
                console.log('Report for ', deviceInfo.path + ':\n', report);
            } catch (e) {
                console.error(e); // ~ "cannot open device with path USB_1050_0120_14200000"
                // if we can't open it, it's not compatible (we say).
            } finally {
                // might be too early to close, but seeing what we can do
                if(rawDevice && rawDevice.close){
                    rawDevice.close();
                }
            }
          }
          ...
          deviceU2fSupportMap[getDeviceKey(deviceInfo)] = {
    ...
    // now that we have updated our cache, return all of the devices
    // from the cache
