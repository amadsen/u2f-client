...
U2FHIDDevice.enumerate = function enumerate(acceptFn) {
    //   v-- switch hid.devices() for U2fDevices.devices() to get a pre-filtered
    //       list of devices with the right usagePage & usage.
    var allU2fDevices = U2fDevices.devices(),
        compatibleDevices = [];

    if(allU2fDevices.length > 0){
      console.log('Checking new devices for u2f devices:\n', newDevices);
      compatibleDevices = allU2fDevices.filter(function(deviceInfo) {
          deviceInfo.id = deviceInfo.path; // Add unique identifier to deviceInfo to help distinguishing them.
          var isCompatible = true; // Assume compatibility because U2fDevices.devices() returned it.
